...
// @desc    Send notification email to seller about their product
// @route   POST /api/products/admin/notify-seller
// @access  Private/Admin
const notifySellerAboutProduct = async (req, res) => {
  try {
    ...
    // Get seller details
    const seller = await userModel.getUserById(product.seller_id);
    if (!seller) {
      return res.status(404).json({
        success: false,
        message: 'Seller not found'
    ...
    await emailService.sendAdminNotificationToSeller(
      seller.email,
      subject,
      message,
      product.name
    ...
    res.status(500).json({
      success: false,
      message: 'Error sending notification to seller',
      error: error.message
    });
  }
};
