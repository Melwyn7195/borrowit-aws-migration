...
async function createOrder(user, payload = {}) {
  ...
  // Parse rental period as integer (days)
  const rentalPeriod = productPayload.rentalPeriod 
    ? (typeof productPayload.rentalPeriod === 'number' 
        ? productPayload.rentalPeriod 
        : parseInt(productPayload.rentalPeriod, 10) || 1)
    : 1;
  
  // Calculate subtotal: unit price (per day) * quantity * rental period (days)
  const subtotal = payload.subtotal !== undefined
    ? safeNumber(payload.subtotal)
    : unitPrice * quantity * rentalPeriod;
...
async function updateOrderStatus(seller, orderNumber, updates = {}) {
  ...
  console.log('updateOrderStatus called with:', { orderNumber, updates });
  ...
  // Handle totalAmount update (for final price after inspection)
  if (Object.prototype.hasOwnProperty.call(updates, 'totalAmount')) {
    const totalAmount = safeNumber(updates.totalAmount);
    setFragments.push(sql`total_amount = ${totalAmount}`);
  }
  
  // Handle pricePerDay update (for product price after inspection)
  if (Object.prototype.hasOwnProperty.call(updates, 'pricePerDay')) {
    console.log('pricePerDay received from frontend:', updates.pricePerDay);
  }
  ...
  const result = await sql`
    ...
    WHERE order_number = ${orderNumber}
      AND seller_id = ${seller.user_id}
    RETURNING order_number, product_id
  `;
  ...
  // If productCondition is provided and status is being set to 'completed', update the product
  console.log('Checking if product should be updated:', {
    statusValue,
    hasProductId: !!result[0]?.product_id,
    productId: result[0]?.product_id,
    hasProductCondition: !!updates.productCondition,
    hasPricePerDay: updates.pricePerDay !== undefined,
    pricePerDay: updates.pricePerDay
  });
  
  if (statusValue === 'completed' && result[0].product_id) {
    const productUpdates = {};
    
    if (updates.productCondition) {
      productUpdates.condition = updates.productCondition;
    }
    
    // If pricePerDay is provided directly, use it
    if (updates.pricePerDay !== undefined) {
      productUpdates.price_per_day = Number(updates.pricePerDay);
    }
    ...
    if (Object.keys(productUpdates).length > 0) {
      ...
      try {
        ...
        const updateParts = [];
        if (productUpdates.condition !== undefined) {
          updateParts.push(sql`condition = ${productUpdates.condition}`);
        }
        if (productUpdates.price_per_day !== undefined) {
          updateParts.push(sql`price_per_day = ${productUpdates.price_per_day}`);
        }
        
        if (updateParts.length > 0) {
          const setClause = updateParts.reduce((acc, part, idx) => 
            idx === 0 ? part : sql`${acc}, ${part}`
          );
          
          const productUpdateResult = await sql`
            UPDATE "Product"
            SET ${setClause}
            WHERE product_id = ${result[0].product_id}
              AND seller_id = ${seller.user_id}
            RETURNING product_id, condition, price_per_day
          `;
          
          console.log('Product update result:', productUpdateResult);
          
          if (productUpdateResult.length === 0) {
            console.warn('No product was updated - check product_id and seller_id match');
          }
        }
...
/**
 * Update receiving_info or return_info for an order
 * Both customer and seller can update these fields for their orders
 */
async function updateScheduleInfo(user, orderNumber, updates = {}) {
  if (!user?.user_id) {
  ...
  // Allow both customer and seller to update schedule info
  const result = await sql`
    UPDATE "Order"
    SET ${buildSetClause(setFragments)}
    WHERE order_number = ${orderNumber}
      AND (customer_id = ${user.user_id} OR seller_id = ${user.user_id})
