...
async function searchProducts(filters) {
  ...
  // get_product_avg_rating is a plain per-row function, not an aggregate, so the
  // filter belongs in WHERE. As a HAVING clause Postgres rejected the whole query
  // with "column p.product_id must appear in the GROUP BY clause".
  if (rating) {
    conditions.push(`get_product_avg_rating(p.product_id) >= ${parseFloat(rating)}`);
  }
...
async function countProducts(filters) {
  ...
  // Same as searchProducts: a per-row function filter, so WHERE not HAVING.
  if (rating) {
    conditions.push(`get_product_avg_rating(p.product_id) >= ${parseFloat(rating)}`);
  }
